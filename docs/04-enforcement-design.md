...
### 9.1 Closed-set label drift across audits, and how reviewers should handle it

Section 9 above covers what reviewers see when a stage fails. This sub-section covers a related but distinct case: a stage succeeds, the disposition is unchanged, but a closed-set classifier label emitted by Stage 2 differs across two runs of the same prompt. The 2026-05-26 reaudit observed this on the BEC fixture: the phase 3 archive envelope carried `prompt_summary.topic_label = "messaging_or_outreach"` and the phase 4 reaudit envelope carried `prompt_summary.topic_label = "payment_or_transfer"` on the same prompt, in the same deploy window. Both values are legal under `TOPIC_LABELS`. The L2 disposition was stable across both runs. The variance was confined to the prompt-summary label layer.

This is an operational-handling question -- "what should a reviewer or auditor do when they see label drift" -- not a determinism question. The canonical determinism call (whether Stage 2 closed-set label emission is *supposed* to be deterministic per-prompt, and if not whether the audit-grade vocabulary stability claim in the classifier-display vocabulary memo needs softening) is being adjudicated separately by the policy track in the topic-label determinism memo. The guidance below stands regardless of which way that adjudication lands: the reviewer cases enumerate cleanly without needing the determinism question resolved.

**Why drift is possible in v5 as shipped.** Stage 2 emits the seven closed-set labels (`topic_label`, `target_label`, `objective_label`, `pretext_label` on `prompt_summary`; Template / Delivery / Control labels on `process_flags[]`) via the same plain-JSON-plus-validator path the Stage 2 evidence object uses -- the model is asked to return a JSON object at temperature 0.1, and the engine post-validates the label strings against the closed-set enums in `coercePromptSummary()` and `coerceProcessFlag()`. Tool-use enforcement (the mechanism Stage 3 uses to constrain L1 / L2 emission) is not applied to the Stage 2 label fields. At temperature 0.1 with a generative model on an ambiguous prompt -- e.g., a BEC envelope that could be reasonably labelled "the prompt is about messaging" or "the prompt is about payment" depending on which surface the model attends to -- some run-to-run variance in the picked label is expected. The validator catches gross typos and out-of-enum strings; it does not pin the choice.

**Cases the reviewer / auditor should distinguish.**

- **Case A -- benign drift (acceptable; reviewer makes a note but does not escalate).** L2 disposition is stable across the two runs. The bright-line list is stable. The aggregate score and component scores are within one integer step. Only the prompt-summary or process-flag closed-set labels (or the prose Explanation fields, or the `target_attributes` membership) differ, and the differing labels are both substantively defensible on the same prompt. This is the BEC case observed in the 2026-05-26 reaudit: both `messaging_or_outreach` and `payment_or_transfer` describe a BEC email that asks for a wire transfer, and the L2 (`phishing_attack`), the bright-line set, and the disposition (`block`) were stable. **Reviewer action:** capture both labels in the audit report when the audit's purpose is vocabulary stability; otherwise no action. Do not re-run the prompt to break the tie -- the tie is the artifact. Do not escalate to a determinism-bug ticket on case-A drift alone.

- **Case B -- drift-with-disposition-shift (quality signal; escalate).** The L2 differs between the two runs, OR the disposition action verb differs, OR the bright-line set changes membership, OR the aggregate score moves by more than two integer steps. In these cases the label drift is downstream of a more substantial Stage 2 / Stage 3 selection difference, and the closed-set labels are the visible symptom of an upstream change in how the prompt was read. **Reviewer action:** escalate as a Stage 2 / Stage 3 determinism quality issue. File against the policy track if the L2 / L1 selection itself drifted (selection-rule question); file against the ops + vscode tracks if the bright-line set differs (the bright-line emission contract is supposed to be evidence-driven, not sampling-driven). Capture both envelopes in full -- not just the differing fields -- so the upstream cause can be traced.

- **Case C -- multi-label drift on a single envelope pair (quality signal; escalate).** L2 disposition is stable, but three or more of the closed-set label fields (`topic_label`, `target_label`, `objective_label`, `pretext_label`, plus the Template / Delivery / Control labels) all differ between the two runs. Single-label drift on an ambiguous prompt is case A; many-label drift on the same prompt indicates Stage 2's evidence reading is unstable in a way the disposition path is fortunate to absorb. **Reviewer action:** escalate as a Stage 2 calibration concern. The disposition holding does not make the underlying read stable.

- **Case D -- explanation-prose drift only (acceptable; no action).** The closed-set labels are identical across both runs, but the prose Explanation fields (`topic_explanation`, `pretext_explanation`) differ in wording. The prose fields are *generative narrative*, not closed-set vocabulary; they are not subject to the audit-grade vocabulary stability claim. **Reviewer action:** no action. Do not capture prose-only differences in the vocabulary-stability section of an audit report.

**Reviewer / auditor SOP for the audit-report write-up.** When a phase-N audit re-runs a fixture that an earlier phase already captured, and the live envelope differs from the archived envelope on closed-set labels, the auditor includes a "label drift" sub-section in the audit report that names: (i) the differing fields, (ii) whether L2 / disposition / bright-lines were stable (case A vs case B/C), (iii) the auditor's case assignment, and (iv) whether the drift is being escalated. Case-A entries can be one paragraph; case-B and case-C entries should carry both full envelopes as cited evidence (envelope id, fetch timestamp, and the differing fields in a side-by-side table). The 2026-05-26 reaudit's §6 P1-D entry on the BEC `topic_label` drift is the canonical format example: brief, names both values, assigns the case (it was case A), recommends without blocking.

**What this sub-section does not do.** It does not pre-judge the policy adjudication on whether closed-set label emission is supposed to be deterministic. If policy converges on "should be deterministic" and queues a Stage 2 retune (e.g., switching the label-emission path to tool-use enforcement, or constraining via a separate Stage 2.5 selection step), case A becomes rarer but the reviewer SOP for cases B / C / D is unchanged -- those cases are about the *substantive* read shifting, not the *label-selection* layer, and they survive any Stage 2 tightening. If policy converges on "expected variance" and softens the vocabulary memo's audit-grade stability claim, case A is named as expected behavior in the memo; the reviewer guidance above is still the canonical operational handling. Either way, the four-case decomposition is the load-bearing piece for ops; the determinism call is the load-bearing piece for policy.
