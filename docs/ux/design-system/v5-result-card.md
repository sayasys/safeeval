...
13. **Add the new section 2.4a (Prompt summary)** in `src/app/evaluator/page.js`, between the existing Triggered-by block and the Evidence panel collapsible. Default-visible (not collapsible).
...
15. **Tooltip-descriptor dictionary.** Add a `CLASSIFIER_LABEL_DESCRIPTIONS` constant in `src/app/evaluator/page.js`, structured as `{ Template: { synthetic_document: "...", ... }, Delivery: {...}, Control: {...}, Topic: {...}, Target: {...}, Objective: {...}, Pretext: {...} }`. Each value is the vocabulary-memo descriptor verbatim. Same pattern as the existing `BRIGHT_LINE_DESCRIPTIONS`, `L1_DESCRIPTIONS`, `L2_DESCRIPTIONS`, `RULE_DESCRIPTIONS`.
...
## 19. Mode-switch UI (extends `src/app/evaluator/page.js` header chrome)

The existing single-prompt page (`src/app/evaluator/page.js`) renders a textarea + Evaluate button at the top of the result region. This phase commits a **mode toggle** that sits immediately above the input area, switches between `"Evaluate a prompt"` and `"Evaluate a conversation"`, and maps one-to-one to the envelope's discriminator (`input.kind`).
...
- More code paths in `src/app/evaluator/page.js`, more divergent design-system doc.
...
Surface: page header above the input region. File: `src/app/evaluator/page.js`. Envelope coupling: `?mode=conversation` URL parameter wired to a `mode` React state; on Evaluate, `input.kind` is set from `mode`. Visual chrome: segmented control per §19.1, default-active `prompt` (§19.2). Confirm dialog wiring for Scenarios B/C/D (§19.3). `history.replaceState` to keep URL in sync (§19.4). ARIA: `role="tablist"`, `role="tab"`, `aria-selected`, `aria-controls`, arrow-key nav, polite-region switch announcement (§19.5). WCAG: contrast live-verified per §22.1 methodology; segmented control fits at 375px viewport with label-narrowing below 420px (§22.2).

**Item 2 -- Sub-modality segmented control (Upload screenshot vs Paste text).**
Surface: inside conversation-mode input region, above the upload zone / textarea. File: `src/app/evaluator/page.js`. Envelope coupling: `input.conversation.modality: "image" | "text"` set from this control (§20.1 + §20.2). Visual chrome: smaller segmented control per §20.2. Same ARIA pattern as Item 1.

**Item 3 -- Image upload zone with drag-and-drop + file picker.**
Surface: conversation-mode + `modality === "image"`. File: `src/app/evaluator/page.js`. New helper file may be split out: `src/components/ImageUploadZone.jsx` if `page.js` size becomes unwieldy. Visual chrome per §20.1. File format: PNG / JPG / HEIC; max 10 MB. Loading state: cancellable spinner during Stage 0; 30s timeout (§20.1).

**Item 4 -- Text upload textarea with .txt file picker fallback.**
Surface: conversation-mode + `modality === "text"`. File: `src/app/evaluator/page.js` (or `src/components/ConversationTextInput.jsx`). Visual chrome per §20.2. Max 100 KB for .txt upload. "Show formatting tips" disclosure per §20.2.
...
Surface: conversation-mode + post-Stage-0. File: `src/app/evaluator/page.js` (or `src/components/ConversationPreview.jsx`). Visual chrome per §20.3. Per-turn cards with editable sender + text, delete + insert affordances (icon-buttons on desktop, overflow menu on mobile per §22.2). Confirm CTA invokes Stages 1-4. Sender canonicalization auto-mapping per §20.4 table (uses `output.modality_hint` from Stage 0).
...
**Item 1 -- Tooltip portal refactor (issue 1).** Surface: chip-tooltip component (likely `ClassifierLabelChip` or `HoverChip` per §13.5 reference). File: `src/app/evaluator/page.js` or extracted component. Apply `ReactDOM.createPortal(tooltipNode, document.body)` per §28.1. Preserve pointer-triangle positioning, direction-flip, scroll/resize re-anchoring, a11y semantics. DOM verification per §28.3.

**Item 2 -- Per-turn-card overflow-menu (issue 2 / §22.2 implementation).** Surface: preview-confirm step's per-turn card. File: `src/app/evaluator/page.js` or extracted component (likely `PreviewTurnCard` or similar). At <768px: hide the existing icon-button stack; render a single three-vertical-dots button per §29.3; tap opens popover with three labeled items (Insert turn above / Insert turn below / Delete this turn). At >=768px: existing icon-stack pattern unchanged. Aria + dismiss + focus management per §29.3 / §29.5.
...
**Item 4 -- L3 chip mobile reflow (issue 3).** Surface: §2.3 Classification envelope's L3 row. File: `src/app/evaluator/page.js` (the L3-row rendering block). At <768px: vertical-stack the category-name above the chip row; add `flex-wrap`, `min-w-0`, `overflow-wrap: anywhere`, `whitespace-normal`, `max-w-full` per §30.2. At >=768px: existing layout unchanged. Match §13.1 vertical-stack visual rhythm.

**Item 5 -- Grep-before-template-inherit (issue 2 / memo §5.7).** Grep `src/app/evaluator/page.js` for sibling inline-icon-stack patterns that mimic the per-turn-card +/× shape. Reproduce grep result in archive Notes back. Apply same overflow-menu treatment OR document why sibling pattern remains as-is.
...
- **Primary gate:** `mode === 'conversation'`. The pill row MUST NOT render in prompt mode under any circumstance. Prompt mode has its own pill row (`EXAMPLE_PROMPTS` at `src/app/evaluator/page.js` line 26 + render at lines 1994-2003); the two pill rows are mutually exclusive at the `mode` gate.
...
- **Render gating.**
  ...
  - Pill row does NOT render when `mode === 'prompt'`. Verified by loading `/evaluator?mode=prompt` (or no param) and asserting `EXAMPLE_CONVERSATIONS` button text is not in the DOM.
...
- **No prompt-mode leakage.**
  - Loading `/evaluator?mode=prompt` does not import or reference `EXAMPLE_CONVERSATIONS` in the prompt-mode render tree. Verified by DOM inspection.
...
- Does not implement the pill row -- vscode track owns implementation in `src/app/evaluator/page.js` per the §27.4 click contract + §27.8 acceptance criteria.
...
This makes `+N more` actively worse than just listing all values: it advertises hidden information while preventing access to it. The TARGET row in the PROMPT SUMMARY section showed six visible chips followed by an inert `+3 more` pill. The two surfaces that carry this bug are `TargetAttributesStrip` (`src/app/evaluator/page.js` line ~2093, cap 5) and `ProcessFlagRow`'s Control multi-label strip (`src/app/evaluator/page.js` line ~2185, cap 4).
...
**Implementation guidance.** The component lives in the same file as the existing chip components (`src/app/evaluator/page.js`) unless vscode chooses to extract chip components to a sibling module; that extraction is out-of-scope for this brief but flagged as a reasonable follow-up if `src/app/evaluator/page.js` continues to grow.
...
10. **ASCII discipline.** `src/app/evaluator/page.js` stays ASCII-only per CLAUDE.md (em dashes -> `--`, smart quotes -> straight, etc.). Verified by `scripts/check-null-bytes.js` (existing pre-flight) plus a grep for non-ASCII codepoints in the changed lines.
...
- **Brief 0045** (`disclose-hidden-values-plus-n-more-chip`) -- the originating dispatch.
