...
| `design`          | IA / system / copy drift: IA conflations (two affordances that look the same but mean different things), design-system drift (variants authored outside `docs/ux/design-system/`), a11y patterns lacking shared abstraction, copy-register inconsistencies, mobile-vs-desktop spec gaps, README accuracy drift against shipped behavior | per dispatch  |
| `qa`              | Structural patterns across audits: a finding shape recurring in 3+ audits that no per-affordance fix can address; audit-level patterns visible only because qa consumes multiple audits as input | per audit     |
