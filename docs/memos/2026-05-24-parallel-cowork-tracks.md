...
Design specs; qa audits -- per the 2026-05-27 split codified in §4.0 and §4.8. Design does not "fix-and-flag" audit findings inside specs (collapses qa's recommends-only discipline). README.md is design-owned and was rewritten in the 2026-05-28 AI T&S language cleanup; voice rule per `handoff/reference/11-language-cleanup-framing.md`. The visual-direction-implementation seam per §4.2 stands -- design never edits `src/app/page.js`. `editorial-web-style` is appropriate for anthropic.com-matching surfaces (README, portfolio surfaces) but not for utility surfaces. **Disclosure-affordance audit (added fourth atomic amendment 2026-05-28):** on any dispatch that touches a rendered data surface (`src/app/page.js` result-card-rendering code, `docs/ux/design-system/v5-result-card.md`, L3 chip / COMPONENT SCORES / EVALUATING spinner / CLASSIFICATION L3 TAGS rendering, or `docs/ux/copy/**` entries that flow into the card), invoke the disclosure-audit five-question check-list at dispatch closure. The check-list (semantic label, disclosure on truncation, separation between repeated elements, affordance density, hiring-reader 30-second test) is captured verbatim in `handoff/reference/cowork-projects/design.md` under "Disclosure-affordance audit"; the codified skill `design:disclosure-audit` is queued via `handoff/board/pending/0055`. **Hiring-reader frame clause (added fourth atomic amendment 2026-05-28):** the portfolio aim's first-30-seconds test is a load-bearing acceptance criterion on every design output -- self-administered with attestation 2026-05-28 -> 2026-06-10 (calibration window), qa-administered thereafter for result-card-touching dispatches per `handoff/reference/cowork-projects/design.md` hybrid clause; non-result-card dispatches remain self-administered with attestation. **Plain-language review SOP for public-audience surfaces (added eighth atomic amendment 2026-05-29 -- §X.1):** on any dispatch that edits a file under in-scope public-audience paths (per the `safeeval-agents:plain-language-reviewer` skill's scope rules -- landing components, marketing routes, `README.md`, case studies, application materials), the design track MUST invoke `plain-language-reviewer` at dispatch closure and address every `block`-severity violation before commit. `warn`-severity and `nit`-severity findings are advisory -- they may be addressed in the same commit or deferred via an observation in `handoff/board/observations/`, but they do not gate the ship. Block-severity violations gate the ship; an unresolved block-severity finding bounces the dispatch back to design via the orchestrator for a revision turn. The SOP runs in parallel as `local_4ec73927` -- the same firing pattern the design track already uses for `design:disclosure-audit` (per §4.4.3 disclosure-affordance audit clause), so the closure-step cost is one additional skill invocation, not a serial-blocking review pass. **Skill registration (added eighth atomic amendment 2026-05-29 -- §X.2):** `safeeval-agents:plain-language-reviewer` is added to the safeeval-agents lineup with the same shape as the existing agents (`policy-author`, `threat-modeler`, `enforcement-designer`, `stakeholder-communicator`, `classifier-translator`, `design-memo-author`, `threat-intel-watcher`, `bundle-agent-skill`). Filed at the canonical safeeval-agents location alongside the other agents; row added to §4.10 skills-to-tracks mapping. The skill is a review-gate skill (recommends-only at the `warn` / `nit` severities; gate at `block` severity); it never edits the audited file. **Promotion criterion -- review-gate to pre-commit hook (added eighth atomic amendment 2026-05-29 -- §X.3):** if `plain-language-reviewer` block-severity violations are caught at review more than 3 times in a 14-day rolling window (sustained pattern, not a single bad week), the SOP promotes from a design-track review-gate to a pre-commit hook on the in-scope paths. This mirrors the lockstep promotion's evolution-via-data discipline -- the SOP starts at the lightest enforcement level that the data justifies, and tightens only when the data shows the lighter level is insufficient. Promotion authority is `tracks-architect` (the SOP lives in §4.4.3 of the canonical memo, so its enforcement-level changes are amendments); the trigger condition is mechanical (count block-severity catches across `git log` + observations); the design track surfaces the promotion candidacy via the standard observation route once the count crosses 3-in-14-days. **Scope boundary (added eighth atomic amendment 2026-05-29 -- §X.4):** memos under `docs/memos/`, code under `src/lib/`, tests under `__tests__/` or `*.test.js`, and internal docs (handoff briefs, architect amendments, dispatch-prompts, board STATE/digest) are NOT in scope for `plain-language-reviewer`. Internal vocabulary -- "L3 chip", "FAF", "co-emission", "lockstep", "default-accept", "route-to-steven", "dispatch closure" -- is intentional in those surfaces; flattening it to public-voice equivalents would erode the precision the framework depends on. The skill fails closed if accidentally invoked on out-of-scope paths (returns a single advisory `nit` saying "out-of-scope path; no review run" rather than producing false-positive findings); this protects against an over-eager design dispatch invoking it on a memo it should not be reviewing. **Why this exists:** the landing-page Phase 1-2 ship (commits `3803941` nine-section landing page; `2fc32b6` Tailwind palette extension; `1b638f4` Inter font wiring; `ce0ba5c` nine-component restyle with the sage/cream/coral design system) shipped with internal vocabulary, version numbers, and iteration narration leaking into copy that hiring managers read. The disclosure-affordance audit (added fourth atomic amendment) catches structural / visual disclosure issues on rendered data surfaces but it does not catch voice / vocabulary leakage on copy surfaces -- those are different failure modes with different five-question check-lists. The hiring-reader frame clause (also fourth atomic amendment) names the acceptance criterion but does not name the review mechanism -- it relies on the designer reading the surface as a stranger would, which the landing-page ship showed is insufficient under iteration pressure. The plain-language-reviewer SOP closes the gap with a mechanical review step that fires on every in-scope-path edit. **What NOT to do:** never edits `src/app/page.js` or any hot JS file; never edits `docs/qa/audits/**` or `docs/ux/audits/**` (qa-owned, including the historical pre-pilot directory); never runs an audit on a shipped surface (route to qa via orchestrator); flags policy / ops accuracy concerns via the relevant inbox, doesn't fix them. Observational lens: IA conflation, design-system drift, copy-register inconsistency, disclosure debt (see §4.9).
...
| `safeeval-agents:plain-language-reviewer` | design | Plain-language review on public-audience surface edits (landing components, marketing routes, `README.md`, case studies, application materials -- per the skill's scope rules); review-gate at `block` severity, recommends-only at `warn` / `nit`; fails closed on out-of-scope paths. Added eighth atomic amendment 2026-05-29; filed at the canonical safeeval-agents location alongside the other agents; fires in parallel as `local_4ec73927` at design-track dispatch closure on in-scope-path edits. Promotion criterion to pre-commit hook on in-scope paths if `block`-severity violations are caught more than 3x in a 14-day rolling window (per §4.4.3 promotion clause). |
...
**Sequencing note (2026-05-29):** the third, fourth, fifth, seventh, and eighth atomic amendments are chronologically ordered as follows -- third (2026-05-27, tracks refinement + proactive discovery generalization) -> fourth (2026-05-28, design-track disclosure-debt prevention) -> fifth (2026-05-28, closure-report escalation-field convention) -> seventh (2026-05-28, sec/compl Phase 1 hygiene additions) -> eighth (2026-05-29, design-track plain-language-reviewer SOP for public-audience surfaces -- this entry). The sixth atomic amendment remains scoped but not yet shipped (data-track registration concurrent with the data-track scoping memo at commit `be30894`); sequence numbers are content-keyed, not strictly chronological, so the seventh and eighth ship ahead of the sixth without renumbering. The third and fourth share authoring infrastructure (the per-track lens / per-track instruction mirror surfaces); the fourth and fifth share the same calendar day but address different framework gaps (fourth: missing proactive disclosure check before dispatch close; fifth: missing structural escalation signal at dispatch close so the Dispatch orchestrator can chain or pause cleanly); the seventh addresses three Phase 1 sec/compl hygiene gaps surfaced by the inaugural `compl:` exercise (commit `f62c34c`); the eighth addresses a design-track public-voice review gap surfaced by the landing-page Phase 1-2 ship (commits `3803941` / `2fc32b6` / `1b638f4` / `ce0ba5c`) -- the fourth's disclosure-affordance audit catches structural disclosure issues on rendered data surfaces, but voice / vocabulary leakage on copy surfaces is a different failure mode the fourth did not cover. The fourth and eighth are sibling design-track SOPs (disclosure-affordance + plain-language-review respectively); both extend §4.4.3 and both add rows to §4.10. The plain-language-reviewer skill is itself a sibling amendment authored in parallel as `local_4ec73927`; the skill brief takes pending ID `0082` and this commit-bounce brief takes `0083` per the §4.5.1 grep-before-assign protocol (highest in-use ID at amendment-authoring time was 0081 from commit `746fb4b`; the parallel skill brief was reserved 0082, this brief took 0083).

- 2026-05-29 -- **Eighth atomic amendment (design-track plain-language-reviewer SOP for public-audience surfaces).** A design-track review-gate addition driven by the landing-page Phase 1-2 ship (commits `3803941` nine-section landing page; `2fc32b6` Tailwind palette extension with cream/sage/coral; `1b638f4` Inter font wiring via next/font/google; `ce0ba5c` nine-component restyle with the sage/cream/coral design system) shipping with internal vocabulary, version numbers, and iteration narration leaking into copy that hiring managers read. The framework had no track-owned review for "public voice" on copy surfaces: the fourth atomic amendment's disclosure-affordance audit catches structural / visual disclosure issues on rendered data surfaces but does not catch voice / vocabulary leakage on copy; the hiring-reader frame clause (also fourth) names the acceptance criterion but relies on the designer reading the surface as a stranger would, which the landing-page ship showed is insufficient under iteration pressure. Steven authorized the new skill plus SOP discipline to catch the failure mode proactively. Edits to this memo: (1) §4.4.3 design addendum gains four new bolded clauses (§X.1 Plain-language review SOP for public-audience surfaces -- mandatory `plain-language-reviewer` invocation at dispatch closure on in-scope-path edits with `block`-severity findings gating the ship; §X.2 Skill registration of `safeeval-agents:plain-language-reviewer` mirroring the existing safeeval-agents lineup shape; §X.3 Promotion criterion from review-gate to pre-commit hook on in-scope paths if `block`-severity violations are caught more than 3x in a 14-day rolling window, mirroring the lockstep promotion's evolution-via-data discipline; §X.4 Scope boundary -- memos / code / tests / internal docs are explicitly out of scope, skill fails closed on out-of-scope paths). (2) §4.10 skills-to-tracks mapping gains a row for `safeeval-agents:plain-language-reviewer` clustered with the other design-track skills. (3) Header date bumped to 2026-05-29. (4) This §13 entry + the §sequencing-note prefix updated to add the eighth amendment and to keep noting the sixth is scoped-but-not-shipped. **In-scope paths (per the skill's scope rules):** landing components (`src/components/landing/**`, `src/components/marketing/**`, equivalent paths under `src/app/landing/**`), marketing routes (`src/app/page.js` landing-section JSX, any future `src/app/(marketing)/**` route group), `README.md`, case studies (`docs/case-studies/**` if and when created), application materials (`docs/application/**` if and when created -- this is a portfolio repo so the path may stay empty for now). **Out-of-scope paths:** `docs/memos/**`, `src/lib/**`, `__tests__/**` and `*.test.js`, `handoff/**`, `docs/01-framework.md` / `docs/03-master-policy.md` / `docs/05-classifier-guidance.md` / `docs/ux/**` / `docs/threat-models/**` / `docs/qa/**` / any other internal doc. The skill returns a single advisory `nit` ("out-of-scope path; no review run") if accidentally invoked on an out-of-scope path. **Severity vocabulary (per the skill):** `block` (must address before commit -- gating); `warn` (advisory; address or defer via observation); `nit` (advisory; address opportunistically). **Promotion mechanics:** the design track counts block-severity catches via `git log` + observations over a 14-day rolling window; on crossing >3 catches, surfaces a promotion-candidacy observation via the standard route (`handoff/board/observations/design-YYYY-MM-DD-plain-language-promotion-candidacy.md`); tracks-architect authors the promotion amendment (review-gate -> pre-commit hook) as a separate atomic amendment, mirroring how lockstep was promoted from advisory to CI-enforced. Surfaces touched: this memo (§4.4.3 four-clause extension + §4.10 row addition + §13 entry + sequencing-note prefix + header line 4); no new directories; no new files in this amendment (the skill itself is authored under the parallel `safeeval-agents:plain-language-reviewer` skill brief in pending/0082, not in this amendment); no code surfaces; no paste-to-Cowork-UI step required for the design instruction mirror at `handoff/reference/cowork-projects/design.md` *in this amendment* -- the SOP clauses live in the canonical memo and the session-start ritual reads from the memo, but a follow-up sync of the design instruction mirror is recommended once the skill ships so the mirror carries the SOP body inline (filed as the standard post-amendment paste step for Steven, not a new pending brief). Bundled commit-bounce brief in `handoff/board/pending/0083-vscode-commit-eighth-atomic-amendment.md` (highest in-use ID was 0081 from commit `746fb4b`; the parallel `safeeval-agents:plain-language-reviewer` skill brief was reserved 0082 by the sibling `local_4ec73927` session per the §4.5.1 grep-before-assign protocol -- this brief takes 0083 to leave the immediate-next ID for the sibling). The brief commits the memo edits as a single commit. Out of scope: authoring the `safeeval-agents:plain-language-reviewer` skill bundle itself (that is the sibling `local_4ec73927` session's deliverable, filed under pending/0082); creating any first-use review run against the landing-page surfaces (will fire on the next design-track dispatch that touches an in-scope path); re-litigating the fourth amendment's disclosure-affordance audit (locked 2026-05-28); touching any file beyond the memo and the commit-bounce brief.
