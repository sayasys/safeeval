...
#### Secondary reason codes (modifiers augmenting primary codes, 15 values as of v5.2.4)
...
| `media_likely_synthetic` | The artifact's media payload (image / audio submitted via the `media_artifact` envelope field) scored above the engine's configurable `MEDIA_SYNTHETIC_THRESHOLD` on an upstream synthetic-media detector OR was adjudicated `likely_synthetic` by the Gemini 1.5 Flash reasoning fallback on the ambiguous-band score [0.4, 0.6]. Modifier on impersonation primaries (`executive_impersonation_payment`, `executive_impersonation_synthetic_media`, `family_impersonation_payment`, `government_impersonation_payment`) and on `model_attack_payload` when a synthetic-media payload accompanies a model-attack pretext. Distinct from `synthetic_media_interactive` (real-time deepfake) and `synthetic_media_prerecorded` (clip / voicemail) -- those describe the impersonation modality the textual prompt frames; `media_likely_synthetic` describes detector evidence on an actual attached media payload. Demo-tier emission point per memo `docs/memos/2026-05-28-synthetic-media-detection-implementation-spec.md` section 7.2; the full disposition.reason_codes[] surface lands with the Stage 4 cascade wiring (the §3.13 phase-2 forward reference above), so v5.2.4 emission lives on `envelope.media_detection_result.reason_codes_emitted` until that dispatch closes. |
...
**Ontology 5.2.4 patch addition (2026-05-29, synthetic-media detection Demo-tier emission -- Phase 2):**

§3.13 secondary reason-code table extended with one new entry, `media_likely_synthetic`, per the synthetic-media detection implementation spec at `docs/memos/2026-05-28-synthetic-media-detection-implementation-spec.md` §7.2 and Steven's Q1 / Q3 adjudications on the Phase 2 dispatch. The addition is a single-row value extension to the existing §3.13 secondary closed set (14 -> 15 entries). No new primary code added; no other §3.13 vocabulary touched.

- **Architectural placement.** The new modifier rides on the existing §3.13 secondary surface (reason codes augmenting a primary). Like every other §3.13 secondary, it never stands alone -- emission requires a primary in the same disposition; the primary candidates for media_likely_synthetic are the impersonation primaries (`executive_impersonation_payment`, `executive_impersonation_synthetic_media`, `family_impersonation_payment`, `government_impersonation_payment`) and `model_attack_payload` when a synthetic-media payload accompanies a model-attack pretext. Distinct from `synthetic_media_interactive` and `synthetic_media_prerecorded`: those two describe the impersonation modality the *textual prompt frames* (live deepfake vs. pre-recorded clip); `media_likely_synthetic` describes upstream detector evidence on an *attached media payload* the engine actually classified. The orthogonality matters because the textual-framing-vs-detector-evidence split allows reviewer SOPs to triangulate (a prompt framing a real-time deepfake plus a high-scored attached image is stronger evidence than either signal alone).

- **Steven-locked adjudications baked in.** (a) **Q1 locked: Stage 2 prompt edit deferred out of Demo tier.** The Stage 2 FAF analysis prompt is NOT touched in this dispatch; the spec's §7.1 Stage 2 prose addition is deferred (route-to-steven open question; resolved DEFER). The engine consumes `media_detection_result` via Stage 3 reason-code emission only; the deferral keeps the lockstep boundary on the Stage 2 prompt clean for the Demo tier. (b) **Q3 locked: 0.4-0.6 ambiguous band for Gemini fallback (default-accept).** Steven accepted the Demo-tier narrowing of the scoping memo's 0.3-0.7 band to bound Gemini cost; widening to 0.3-0.7 is a Full-tier amendment.

- **Engine emission point (interim).** The full Stage 4 cascade-rule machinery that emits `disposition.reason_codes[]` is the §3.13 "phase-2 wiring" forward reference (the entry above at the v5.2.1 amendment-log entry) -- that wiring has not landed (no `REASON_CODES_PRIMARY` / `REASON_CODES_SECONDARY` engine constants; no `checkReasonCodeSecondaryLockstep` verifier; no `disposition.reason_codes[]` schema slot). Until that dispatch lands, the v5.2.4 emission is recorded on the synthetic-media-specific surface: `envelope.media_detection_result.reason_codes_emitted: string[]` (added to the `media_detection_result` schema $def alongside the new Phase 2 `reasoning` sub-field). When the §3.13 cascade-rule wiring lands, it MUST read this list and project it into `disposition.reason_codes[]` as a `tier:secondary` entry; this is a forward-compatibility contract recorded here so the eventual wiring dispatch does not re-litigate the architectural placement.

- **Lockstep coverage status.** No engine-side closed-set constant exists for §3.13 yet (per the v5.2.1 amendment-log entry above, the verifiers `checkReasonCodePrimaryLockstep` / `checkReasonCodeSecondaryLockstep` are forward references for the phase-2 wiring). Adding `media_likely_synthetic` to §3.13 therefore does not affect any existing lockstep check today -- the addition is trivially lockstep-GREEN. When the phase-2 wiring lands, the same `checkReasonCodeSecondaryLockstep` shape (`scripts/check-lockstep.js` extracts the §3.13 secondary table by parsing rows starting with `| \`secondary_code_value\` |` and asserts engine equality) will pick up `media_likely_synthetic` automatically because it lives in the same closed-set table the future verifier reads.

- **`ontology_version` decision and rationale.** Patch-bump 5.2.3 -> 5.2.4 on the annotated drafts list (header line); the primary engine-active version stays at `5.2` so the existing `checkV52CaseStudyLockstep` invariant continues to pass. The patch-bump pattern mirrors the v5.2.1 / v5.2.2 / v5.2.3 calls -- vocabulary additions on the policy-side-only / downstream-module surfaces do not warrant a 5.3 minor promotion under the current versioning discipline.

- **Schema bump decision.** No schema bump. The new entry rides on the existing §3.13 secondary closed set (no new vocabulary category); the engine emission point is the new optional `reason_codes_emitted` list on `media_detection_result` and the new optional `reasoning` sub-field, both lifted on the Phase 2 dispatch under the Phase 1 schema delta's `additionalProperties: false` constraint (the additive fields land in the same $def, not as new top-level fields). `schema_version` remains 5.1.

- **Phase scope (forward references).** Out of Phase 2 scope per Steven: `ai_generated_confirmed` bright-line (deferred to Full tier per scoping memo §5.2), video detection (deferred per Demo tier), upload widget UI (Phase 4), real HF / Gemini integration tests against live endpoints (Phase 4), custom fine-tuned detector models (future). The full Stage 4 `disposition.reason_codes[]` cascade wiring (the v5.2.1 amendment-log entry's "phase 2 scope" forward reference) is the next dispatch that consumes `media_detection_result.reason_codes_emitted` and projects it into `disposition.reason_codes[]`; until that dispatch lands, the emission lives on the media-detection-specific surface.

- **Source citations.** Implementation spec: `docs/memos/2026-05-28-synthetic-media-detection-implementation-spec.md` §§4.1 (Stage 0 routing), 6.1 (ambiguous-band Gemini fallback), 7.2 (Stage 3 reason-code emission), 11 (explicit deferrals), 13.1 / 13.3 (Q1 + Q3 default-accept adjudications). Scoping memo: `docs/memos/2026-05-28-synthetic-media-detection-scoping.md` §§3.1 (envelope shape), 4 (detector + Gemini choice), 5.2 (Demo tier scope boundary). Phase 1 commit chain: `1421147` (types + module skeleton), `c790e33` (HF integration), `d89a772` (schema delta), `2342880` (env example).
