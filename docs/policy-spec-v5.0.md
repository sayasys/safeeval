...
### Decision 16 (post-v5.0.1) -- AI-Enabled-Abuse bright-line defender-framing escape valve (cascade Rule 1.5)

- **Question:** Should the bright-lines `prompt_injection_payload` and `ai_model_impersonation` always force `block`, or should defender-framed prompts with no operationalization markers route to `human_review`?
- **Resolution:** SPLIT. The bright-line detection at Stage 2 is unchanged (the evidence emission contract is preserved). The disposition cascade gains a new Rule 1.5 between current rules 1 and 2, scoped to the two AI-Enabled-Abuse bright-lines, conditional on a narrow L3 conjunction (academic / defensive / authorized-pentest context marker + `authorization_unverifiable` risk marker + no operationalization markers). When Rule 1.5 fires, disposition is `human_review`, the forced L2 set becomes `['borderline_security_research', 'borderline_red_team']` under L1 `ambiguous_dual_use`, and the original bright-line evidence remains in `triggered_by.bright_lines` for audit. Implementation references: `docs/04-enforcement-design.md` section 5.1 (canonical cascade entry), `docs/05-classifier-guidance.md` section 4 (bright-line rationale rewrites for both AEA bright-lines), `docs/memos/2026-05-25-policy-case07-defender-framing.md` (adjudication memo).
- **Rationale:** The classifier-guidance text already described the bright-line as a conjunction ("injection framing combined with fraud or deception intent"), and master policy section 3.9 enumerates academic publication as legitimate use. The engine's pre-2026-05-25 behavior implemented neither -- it treated the bright-line as unconditional. Rule 1.5 brings the engine in line with the policy text while preserving the bright-line's audit trail and bounding spoofing via the operationalization-marker carve-out (a prompt that claims defender framing but also names a specific victim, embeds payment instructions, or asks for scale enablement fails the conjunction and routes to `block` per the standard rule). The disposition `human_review` is exactly what the ontology has for cases the machine cannot decide alone. The decision is a partial-adopt: the bright-line stays hard at emission; the cascade gains a narrow, ontology-vocabulary-only routing path. Reconciliation with the existing "non-negotiable" framing in enforcement-design section 5: Rule 1.5 is a deterministic upstream cascade rule gated on L3 surface signals the classifier already emits, not a downstream model re-argument -- the architectural invariant "no downstream stage may downgrade" is preserved.

### Decision 17 (post-v5.0.1) -- Multi-L2 `l2_probabilities` cardinality + Stage 3 within-tolerance tiebreak

- **Question:** Are multi-L2 probability maps in-spec under schema rule 12, and what deterministic rule should Stage 3 apply when two or more L2s are within `L2_PICK_PROBABILITY_TOLERANCE` of the maximum?
- **Resolution:** PARTIAL ADOPT of ops's BEC-map spec memo (`docs/memos/2026-05-25-ops-stage2-bec-map-spec.md`). (a) CONFIRM multi-L2 maps are in-spec; sharpen rule 12 with new sub-rule 12a making cardinality explicit (the map MAY contain one key or multiple keys; only the picked L2 must be one of them). (b) ADD new sub-rule 12b: within-tolerance tiebreak prefers the L2 referenced by a fired bright-line feature; falls back to alphabetical-earliest by ASCII order when zero of the tied L2s is referenced by a fired bright line or when multiple tied L2s are each referenced by different fired bright lines. (c) DEFER symmetric credential-targeting phishing case with named empirical trigger (a credential-capture prompt surfacing the same hybrid-collapse pattern as BEC in a future audit or live-eval). Implementation references: `docs/07-v5-schema.md` section 6 sub-rules 12a and 12b (schema text); `docs/memos/2026-05-25-policy-bec-map-rule12.md` (adjudication memo).
- **Rationale:** Stage 2's job is evidence; Stage 3's job is selection. Multi-L2 maps reflect the FAF's CONTEXT / PROCESS / OBJECTIVE orthogonality at the evidence layer -- forcing a binary at Stage 2 would commit the engine to a policy stance the FAF has not taken. Sharpening the cardinality silence documents existing behavior (the example map in `docs/07-v5-schema.md` section 2 has been multi-key all along) and prevents re-litigation. The bright-line-first tiebreak uses authority that already exists (rule 11's `ai_model_impersonation` bright-line / L2 coupling is the strongest-form precedent; 12b is a weaker form of the same authority applied only within tolerance) and is deterministic across runs. Symmetric phishing case is deferred because its evidence is theoretical -- adding worked examples for unobserved hybrid-collapse patterns risks generalizing the Stage 2 prompt too aggressively (over-emission watch in ops's memo section 8 step 7); the deferred-work trigger is empirical, with ux as the named observation surface.

### Decision 18 (v5.3 phase-1b) -- Four-dimension separation of PROMPT_SUMMARY ontology
...
- **Resolution:** ACCEPT. Add `typology:` (18 values, IC3/FTC-aligned per `docs/08-v5-ontology.md` §3.9) and `persona:` (16 values per §3.10) as new L3 closed-set categories; extend `context_marker:` with 10 channel-origin values (§3.4); reshape the prompt-summary PRETEXT vocabulary to be rationale-only (17 values per §3.11); deprecate the PERSONA prose field from card chrome (envelope-alias retained during 90-day managed dual-emit). Ontology bumps 5.2 -> 5.3 (vocabulary; engine constant rolls in phase 3). `schema_version` unchanged (envelope shape preserved; new L3 categories drop into the existing `classification.l3[]` array). PERSONA_DESCRIPTION prose field omitted for v1. Pig butchering ships as a separate top-level `typology:` value (not hierarchical under romance / investment). Phase 1b scope is vocab-only (Option C per source memo §8.1); Stage 2 system-prompt edits, schema-constant updates, fixture migration, lockstep validator code, and result-card render are deferred to phase 3 (vscode). The three credit-blocked P1 follow-ups (0035 BRIGHT_LINE_FORCED_L2 recovery-fraud drift; 0036 realtime-synthetic-media bright-line celebrity FP; 0037 `overlap:secondary_victimization` Stage 3 FP cleanup) ship surgically against ontology 5.2 first (Path (a)(i)); Phase 1b proceeds to 5.3 after they close. Implementation references: `docs/memos/2026-05-27-four-dimension-ontology-separation.md` (this memo; §8.1 adjudication detail, §10 fixture survey, §11 lockstep extension plan); `docs/08-v5-ontology.md` §§3.4-extension, 3.9, 3.10, 3.11 (closed-set vocabulary tables, inclusion / exclusion criteria, prose-to-label mappings).
- **Rationale:** The conflation produces visible defects on the live result card -- duplicate `lonely_individual` across `target_label` and the `target_attributes` pill row; slash-separated PERSONA prose conflating identity claim and contact channel ("Romantic partner / dating app match"); TOPIC bundling fraud typology (`relationship_grooming`) with prompt shape (`document_generation`). The L3 closed-set discipline already absorbs ten categories; adding two and extending a third is well within the established pattern (the 2026-05-26 vocabulary memo absorbed seven new closed sets at one minor bump). IC3 / FTC reporting taxonomies categorize on fraud typology as the primary axis (IC3 Crime Type tables, FTC Imposter Scams hierarchy); aligning `typology:` with those conventions gives SafeEval a vocabulary external fraud analysts recognize while preserving L2's FAF-policy scope. Per-dimension countability -- the substantive job of any fraud-classification ontology -- requires each conflated axis in its own closed-set field; the four-dimension reshape is the minimum cost to deliver that. NIST IR 8344 treats fraud type as a top-line dimension distinct from method, supporting the axes-design choice. The pig-butchering separate-top-level decision aligns with IC3 reporting that tracks pig butchering as a distinct loss-curve pattern, while the flat-not-hierarchical decision keeps L3 consistent with the eight existing categories (none of which use hierarchical values). Phase 1b vocab-only scope honors the CLAUDE.md venue-boundary discipline: policy lands the policy in Cowork; vscode lands the engine implementation in phase 3.
