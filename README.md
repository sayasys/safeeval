...
> **This repository is the public portfolio cut of SafeEval.** It contains the Fraud Analysis Framework (FAF) documentation, the classification engine, the case-study analyses, and the live evaluator demo. The signed-in SaaS product surface — case management, custom patterns and classifiers, audience-tailored reports, the reviewer feedback loop, the OSINT signal pipeline, and the multi-tenant data layer — lives in a separate private repository. That separation is deliberate: the framework and engine demonstrate the work; the product surface is commercial. The portfolio cut deploys and runs standalone.
...
SafeEval is built as a parallel-tracks framework: policy, design, engineering, QA, and architect each operate as separate sessions with their own briefs, handing off through inbox files and a global state document. The framework memo, the atomic amendments (seven shipped of eight numbered amendments within the framework memo itself), and the public cross-cutting policy memos (PII zero-storage, synthetic media detection) are in `docs/memos/`; the SaaS-side scoping memos (OSINT monitoring, audience-tailored reports, the classifier-edits feedback loop, custom patterns and classifiers, case management) travel with their implementation in the private repository. The implementation work was done end-to-end with Claude and Cursor as the engineering surface, named here because this is an AI policy domain and fluency with that tooling is part of the work.
...
5. [Synthetic media detection scoping memo](./docs/memos/2026-05-28-synthetic-media-detection-scoping.md) — the scoping memo behind the evaluator's unified synthetic-media tab: detector selection, the verdict shape, and how media findings map onto the FAF disposition vocabulary.
